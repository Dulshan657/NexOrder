// Create the Tridon PO-Inbox demo login.
//
//   node tests/fixtures/po-samples/tridon-demo-seed.mjs            create
//   node tests/fixtures/po-samples/tridon-demo-seed.mjs --clean    delete
//
// One Supabase auth user (Admin role) whose email the frontend special-cases
// (lib/demoAccounts.ts → isTridonDemoUser) to lead the sidebar with
// PO Inbox → Order Import → Shop and wear Tridon's logo. Admin role ⇒ no
// horeca_id. Idempotent: re-running is a no-op if the user already exists.
//
// The customer/product/auto-approve data is seeded separately by
// sydney-tools-seed.mjs + repco-seed.mjs (run via `npm run seed:tridon-demo`).
//
...
async function clean() {
  ...
  console.log(`Deleted Tridon demo account "${DEMO_EMAIL}".`)
}
...
async function seed() {
  ...
  console.log(`Tridon demo account ready: ${DEMO_EMAIL} / ${PASSWORD} (role ${DEMO_ROLE}).`)
}
