...
- **M1 · Grid scale cannot be set in the UI.** `mutate-layout` accepts `cell_size_m` but nothing sends it, so every layout is 1.0 m/cell. Scoring normalises min/max, so bin *ranking* is safe — but displayed distances are notional and a 1.2 m bay cannot be drawn true. **What actually matters is keeping scale consistent**; relative error between aisles is what breaks routing, not a uniform one.
- **M2 · The setup order is undocumented** and strictly order-dependent (see the dependency chain). Nothing in the UI hints at it. This document is the mitigation.
- **M3 · `wie_putaway_candidates` is capped at 200 placements**, ordered by dock distance. **Count Amadiya's bays.** If the racked core exceeds 200, the farthest bays are invisible to the engine and stock will never be recommended there.
...
5. **Confirm the bay count** against the 200-candidate cap (M3).
...
1. **Configure before drawing** — storage forms, level roles, zone profiles, `wie_rules`. This is the order-dependent config from the chain above.
