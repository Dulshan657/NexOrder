...
export function WarehouseEmptyState({
  ...
  const sub =
    reason === 'unpublished'
      ...
      : 'Design a layout — or import a floor plan — to turn ' + warehouseName + ' into a visual, bin-level warehouse.'
  ...
  return (
    // Centering wrapper only — the hero card below keeps `.glass-panel
    // shadow-card` (this replaces the whole page, no map behind it, so it
    // doesn't switch to `.map-panel`). WarehousePage's content slot is
    // `md:flex-1 md:min-h-0`, a block box, so without this the card would
    // just sit at the top instead of centering in the available height.
