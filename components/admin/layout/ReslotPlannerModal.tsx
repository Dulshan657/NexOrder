...
import { ArrowRight, Loader2, TriangleAlert, PackageCheck, X } from 'lucide-react'
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4" onClick={onCancel}>
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-stone-200 px-5 py-3.5">
          <div>
            <h3 className="text-sm font-semibold text-stone-800">Re-slot existing stock</h3>
            <p className="text-xs text-stone-500">{warehouse.name} — review where stock moves in the new layout, then publish.</p>
          </div>
          <button onClick={onCancel} className="rounded-md p-1 text-stone-400 hover:bg-stone-100 btn-press" aria-label="Cancel">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-stone-200 px-5 py-3">
          <button onClick={onCancel} disabled={publishing} className="rounded-lg border border-stone-200 px-3 py-1.5 text-xs font-medium text-stone-600 hover:bg-stone-50 disabled:opacity-50 btn-press">
            Cancel
          </button>
          <button
            onClick={() => onApprove(commitMoves)}
            disabled={!feasible || publishing}
            title={!feasible ? 'Resolve capacity before publishing' : moves.length ? 'Publish and create the relocation worklist' : 'Publish'}
            className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-4 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-emerald-500 disabled:opacity-50 btn-press"
          >
