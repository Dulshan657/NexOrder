...
/** Every field that is promotion data. `productSearch` is a transient filter and is
 *  deliberately absent, so typing in the search box never counts as an unsaved edit. */
interface FormSnapshot {
    ...
    targetingKind: PromotionTargeting['kind'];
    targetHoReCaIds: number[];
    targetTiers: HoReCaTier[];
    targetRepId: string;
...
const PromotionForm: React.FC<PromotionFormProps> = ({ promotion, products, hoReCas, users, onSave, onClose }) => {
    ...
    const snapshot: FormSnapshot = {
        name, description, type, percentOff, fixedPrice, clearancePercent,
        bogoBuyProductId, bogoBuyQty, bogoGetProductId, bogoGetQty, bogoSameProduct,
        bundleProductIds, bundlePrice, appliesTo,
        scopeKind, scopeProductIds, scopeCategories,
        targetingKind, targetHoReCaIds, targetTiers, targetRepId,
        stackWithHoReCaPricing, startDate, endDate, minOrderValue, priority,
    };
    // Captured from the first render rather than re-derived from `promotion`, so the
    // baseline cannot drift away from the useState initialisers above.
    const [initialSnapshot] = useState(() => snapshot);
    const isDirty = (Object.keys(initialSnapshot) as (keyof FormSnapshot)[]).some(
        key => !sameValue(snapshot[key], initialSnapshot[key])
    );
    ...
    return (
        <Modal
            ...
            size="2xl"
            dirty={isDirty}
            icon={<Tag className="w-4 h-4 text-nexgen-blue" />}
                        ...
                        <div className="mb-3">
                            ...
                            <div className="flex gap-2">
                                {(['unit', 'carton'] as const).map(u => (
                                    <button key={u} type="button" onClick={() => setAppliesTo(u)}
                    ...
                    {type === 'bundle' && (
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                {/* Own scroller: a 30-row picker must not push the price field it sits under off-screen. */}
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 mb-3">
                            {(['storewide', 'products', 'categories'] as const).map(k => (
                                <button key={k} type="button" onClick={() => setScopeKind(k)}
                        ...
                        {scopeKind === 'products' && (
                            <div>
                                <input placeholder="Search products..." value={productSearch} onChange={e => setProductSearch(e.target.value)} className={`${inputClass} mb-2`} />
                                {/* Own scroller — same reason as the bundle picker above. */}
                                <div className="max-h-40 overflow-y-auto border border-stone-200 rounded-lg divide-y divide-stone-100">
                                    {filteredProducts.slice(0, 30).map(p => (
                                        <label key={p.id} className="flex items-center gap-2 px-3 py-2 hover:bg-stone-50 cursor-pointer">
                        ...
                        {scopeKind === 'categories' && (
                            <div className="flex flex-wrap gap-2">
                                {categoryOptions(products).map(cat => (
                                    <button key={cat} type="button" onClick={() => setScopeCategories(prev => prev.includes(cat) ? prev.filter(c => c !== cat) : [...prev, cat])}
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                        {([['all', 'All HoReCa'], ['horecas', 'Specific Customers'], ['tier', 'By Tier'], ['rep', 'Per Sales Rep']] as const).map(([k, label]) => (
                            <button key={k} type="button" onClick={() => setTargetingKind(k)}
                    ...
                    {targetingKind === 'horecas' && (
                        /* Own scroller: the customer list is unbounded and sits above the date fields. */
                    ...
                    {targetingKind === 'tier' && (
                        <div className="flex gap-2">
                            {TIER_OPTIONS.map(t => (
                                <button key={t} type="button" onClick={() => setTargetTiers(prev => prev.includes(t) ? prev.filter(x => x !== t) : [...prev, t])}
