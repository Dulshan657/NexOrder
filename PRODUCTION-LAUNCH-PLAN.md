...
| Site | Change |
|---|---|
| `vercel.json:9` | CSP: list **both** Supabase hosts in `img-src`/`connect-src`, and **add `frame-src 'self' blob: <hosts>`** — currently absent, so it falls back to `default-src 'self'` and would break `POInboxDetailModal.tsx:718` and `DocumentViewerContext.tsx:124` the moment CSP is promoted to enforcing. Convert to `vercel.ts` in Phase C. **Do not** diverge `vercel.json` between branches — it conflicts on every promotion. |
