...
- **`meta.zoneProfileId` is INTENT, not a binding.** A bin inherits its zone by **materialized-path ancestry to a `kind='ZONE'` location** (`00047`'s header; `wie_putaway_candidates`' LATERAL join) — `locations.zone_profile_id` *on a bin* is read by nothing, so stamping it would be a no-op dressed as a feature. See Pending Work.
...
| ↳ `layout_objects.meta` (an AREA's name only) | `mutate-warehouse-location` `rename_area` | Admin, Manager | `00094` |
