...
- **`meta.zoneProfileId` is INTENT, not a binding.** A bin inherits its zone by **materialized-path ancestry to a `kind='ZONE'` location** (`00047`'s header; `wie_putaway_candidates`' LATERAL join) — `locations.zone_profile_id` *on a bin* is read by nothing, so stamping it would be a no-op dressed as a feature. See Pending Work. Live re-tinting (`00095`) does not change this: it edits the same `meta` field and binds nothing.
...
**Medium**
3. **Bind a named area's bins to its ZONE** — the operational half of mig `00090`, deliberately deferred (2026-08-03) so the visual layer could ship on its own. **More reachable since `00095`:** an operator can now paint and re-tint a zone-profiled area on a *live* site, so the gap between what the map promises and what putaway does is now something they can create in one click. An area records `meta.zoneProfileId`, but nothing reads it: a bin inherits its zone by **materialized-path ancestry to a `kind='ZONE'` location**, so making an area real means **re-parenting every bin under it** — `mutate-layout`'s `resolveZone` already find-or-creates the ZONE row, but it only sets `parent_id` at CREATION and there is no re-parent path for a saved bin.
   - **Why this is its own change, not a side effect of a geometry save.** `locations` carries **no trigger** (verified — `pg_trigger` is empty for it), so `materialized_path` is maintained entirely by hand. A move must rewrite the bin's path *and* every SHELF child of a levelled rack, plus the reverse when an area is renamed, erased or shrunk. Get it wrong and the bins fall out of `getWarehouseLocations`' `LIKE '<wh>/%'` prefix query — which is precisely the "map all grey, empty Locations tree" failure mode fixed in `6a253b2`, and they also vanish from `wie_putaway_candidates`.
   - Scope it as: re-parent on save, reverse on removal, one migration for any backfill, and verify with the rollback-isolated `apply-sql.mjs BEGIN…ROLLBACK` pattern before touching MAIN.
4. **Desktop entry point for a stocktake** — `count-bin` and the Stocktake page ship phone-first (scan a bin, count it). The office-side case — reconciling against a paper count, or correcting one bin noticed while looking at the map — still has only `AdjustStockModal`. Scope it as a "Count this bin" action on `BinDetailPanel` (`components/inventory/warehouse/BinDetailPanel.tsx`) and on the Stock page, opening the same `CountSheet` in a `<Modal>`. No server work: `count-bin` already takes any location.
5. **Accessibility pass** — minimal ARIA. Add labels to icon-only buttons (`AccountsAgingTable`, sort headers, modal close), focus traps in `BundleSelectModal` / `OrderVerificationModal`, ARIA-live region for the toast container.
6. **Email expansion** — wire `invoice_issued` template on invoice → `issued`; decide whether to use the custom `user_invitation` template vs Supabase's built-in invite email.
7. **Test coverage expansion** — strong PO-inbox, pricing, scan, auth-link and WIE-engine coverage; PO-inbox matching resolvers use the `__tests__/support/fakeSupabase.ts` harness. Gaps: cart submission flow, pantry add/remove, HoReCa reason-prompt gate, role-based routing.
